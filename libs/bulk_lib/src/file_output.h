...
class FileOutput : public std::enable_shared_from_this<FileOutput>, public IObserver
{
    ...
    static constexpr std::string_view m_underscore = "_";
...
public:
    ...
    {
        observable->subscribe(std::move(shared_from_this()));
    }
    ...
    void update(const std::string_view message) override
    {
        ...
        filename << m_filename_prefix << m_underscore << std::this_thread::get_id() << m_underscore;
        filename << timestamp << m_extension;
